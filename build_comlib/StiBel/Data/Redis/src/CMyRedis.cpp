...
static CMyRedis::ptr  CMyRedis::getInstance(const string& strIp,int nPort,const string& passwd)
{
	if(m_MyRedis ==NULL)
	{
		...
		m_MyRedis->Connect(strIp,nPort,passwd);
	}
	return m_MyRedis;
}
...
void CMyRedis::Connect(const string& strIp,int nPort,const string& passwd)
{
		...
		m_pRedisReply = (redisReply*)redisCommand(m_pRedisContext,"AUTH %s", passwd.c_str());
		if(m_pRedisReply == NULL)
		{
			freeReplyObject(m_pRedisReply);
			throw CErrorMsg("auth error! m_pRedisReply is NULL");
		}
